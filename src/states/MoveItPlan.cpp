...
void MoveItPlan::start(mc_control::fsm::Controller & ctl_)
{
  ...
  else { frame_ = &ctl.robot().frame(efFrameName); }
...
void MoveItPlan::setup_interface(mc_control::fsm::Controller & ctl, const std::string & mode)
{
  ...
  ctl.gui()->addElement({"MoveIt"}, {},
                        ...
                        mc_rtc::gui::Label("State", [&]() -> const std::string & { return status_; }),
                          ...
                          mc_rtc::gui::Button("Set posture target",
                                              [&]()
                                              {
                                                for(size_t i = 0; i < ctl.robot().mb().joints().size(); ++i)
                                                {
                                                  ...
                                                  if(j.dof() == 1)
                                                  {
                                                    posture_target_[j.name()] = ctl.robot().mbc().q[i];
                                                  }
                                                }
                                              }),
                          mc_rtc::gui::Button("Plan", [&]() { make_posture_plan(); }));
  }
  ctl.gui()->addElement({"MoveIt"}, {}, mc_rtc::gui::Button("Execute", [&]() { execute_plan(ctl); }),
                        ...
                        mc_rtc::gui::Form(
                            "Add obstacle", [&](const mc_rtc::Configuration & config)
                            { add_obstacle(ctl, config("Name"), config("Size")); },
